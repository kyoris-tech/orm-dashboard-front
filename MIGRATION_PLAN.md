...
## 10.34 Candidatura pública vinculada a processo seletivo, com percentual de adesão

- **Backend**: quando alguém envia um currículo pela página pública da vaga (`POST /public/job-openings/:code/apply`), o currículo agora é automaticamente atrelado a um processo seletivo daquela vaga:
  - `JobOpeningService.getOpenJobOpeningForApply` (renomeado/ampliado de `getCompanyIdForOpenPublicCode`) passou a retornar também `id`, `title`, `requirements` e `differentials` da vaga, não só o `companyId`.
  - `SelectionProcessService.attachPublicApplication(jobOpening, resume)` (novo): procura um processo seletivo `OPEN` já vinculado àquela vaga; se não existir, cria um chamado `"Candidaturas — {título da vaga}"`. Depois adiciona o currículo como candidato (via `upsert`, seguro contra duplicata se o candidato reenviar).
  - `PublicJobOpeningController.apply()`: depois do upload/análise por IA, chama `attachPublicApplication` — sem alterar a resposta do endpoint pro candidato.
  - **Percentual de adesão** (`SelectionProcessCandidate.matchScore`, novo campo `Int?`, migração `20260819011041_selection_process_candidate_match_score`): compara os `requirements` + `differentials` da vaga (texto livre) contra os dados extraídos do currículo (`skills`, `role`, `summary`, `qualifications`, `courses`, descrições de `experience`) — cada requisito conta como "atendido" se pelo menos metade das palavras dele aparece no currículo; o percentual é `requisitos atendidos / total de requisitos`. Heurística por texto, sem custo de IA extra (mesma abordagem não-IA já usada em "Analisar Candidatos" pra compatibilidade com filtros de busca). Se a vaga não tiver requisitos/diferenciais cadastrados, `matchScore` fica `null` (sem nota, em vez de inventar um número).
  - O mesmo cálculo passou a ser aplicado também quando um recrutador cria um processo já vinculado a uma vaga (`create`) ou adiciona candidatos a um processo com vaga vinculada (`addCandidates`) — pra não ficar inconsistente (candidato de candidatura pública com nota, candidato adicionado manualmente sem nota, no mesmo processo).
- **Frontend**: `SelectionProcessDrawer.tsx` (o "abrir o processo") agora mostra, ao lado do nome de cada candidato, um badge "`N% de adesão`" quando há nota — reaproveitando a mesma paleta de cores de "Analisar Candidatos" (verde >80%, azul 60-80%, amarelo 30-60%, cinza <30%).
- **Achado no caminho**: o `.env` local do backend tinha `DATABASE_URL` duplicado, com a segunda ocorrência apontando pro banco de produção do Railway (provavelmente colado por engano numa sessão de debug anterior) — isso fazia `prisma migrate dev` tentar conectar em produção. Removida a duplicata, mantendo só a `DATABASE_URL` local (`localhost:5433/orm_dev`). Arquivo não versionado (`.gitignore`), sem impacto em produção.
- **Validação**: `tsc --noEmit` + `nest build` (backend) e `eslint` + `tsc --noEmit` + `next build` (frontend) limpos. Testado de ponta a ponta com um currículo sintético (sem usar dados de terceiros): enviei duas candidaturas pra mesma vaga com requisitos `React, TypeScript, Python` + diferencial `AWS` — confirmado que reaproveita o mesmo processo (não duplica), e que os dois candidatos ficaram com `matchScore: 50` (2 de 4 critérios batendo: React e TypeScript), exibido corretamente como badge amarelo "50% de adesão" no drawer do processo. Dados de teste removidos do banco local depois.

## 10.35 Limpar input após candidatura, campo de benefícios na vaga e badge de adesão sempre visível

- **Limpar input de arquivo após candidatura bem-sucedida**: [PublicApplyArea.tsx](src/features/public-job-opening/components/PublicApplyArea.tsx) — o `onSuccess` da mutation de candidatura agora reseta o estado `file` e o `value` do `<input type="file">` (via ref), voltando a área pro estado "Selecionar arquivo" (a mensagem de sucesso continua aparecendo). Testado com um upload real: confirmado que `input.value`/`input.files` zeram e o heading volta para "Selecionar arquivo" depois do processamento.
- **Campo "Benefícios" em criar/editar vaga**: como a tela de **editar** vaga não existia ainda (só criar + cancelar), implementei a feature completa de edição:
  - **Backend**: novo campo `JobOpening.benefits String[]` (migração `20260819013009_job_opening_benefits`); `UpdateJobOpeningDto` (novo) + `PATCH /api/v1/job-openings/:id` + `JobOpeningService.update()` com log de auditoria (`UPDATE`); `findPublicByCode` e a criação passaram a incluir `benefits`.
  - **Frontend**: `CreateJobOpeningDialog.tsx` virou [`JobOpeningFormDialog.tsx`](src/features/job-openings/components/JobOpeningFormDialog.tsx), um diálogo único para criar/editar (mesmo padrão do `PlanFormDialog`), com um terceiro `TagListInput` de "Benefícios" ao lado de Requisitos/Diferenciais. Na edição, a faixa salarial atual é mostrada como texto informativo — os dois `CurrencyInput` só substituem o valor se preenchidos (evita perder a faixa salarial ao editar sem querer mexer nela, já que ela é salva como string opaca, não como dois números).
  - Botão **"Editar vaga"** novo no [`JobOpeningDrawer.tsx`](src/features/job-openings/components/JobOpeningDrawer.tsx), que também passou a exibir os benefícios (badges verdes) junto de requisitos/diferenciais.
  - A página pública da vaga ([`PublicJobOpeningView.tsx`](src/features/public-job-opening/components/PublicJobOpeningView.tsx)) também passou a listar os benefícios pro candidato.
- **Badge de adesão sempre visível, mesmo em 0%**: a pedido do usuário, [`SelectionProcessDrawer.tsx`](src/features/selection-processes/components/SelectionProcessDrawer.tsx) agora sempre mostra o badge "`N% de adesão`" (usando `matchScore ?? 0`), em vez de escondê-lo quando `matchScore` é `null` (candidato sem vaga vinculada ao processo) — seguindo a mesma lógica de "Analisar Candidatos", que sempre mostra o percentual mesmo quando é 0.
- **Validação**: `tsc --noEmit` + `nest build` (backend) e `eslint .` + `tsc --noEmit` + `next build` (frontend) limpos, 0 erros. Testado de ponta a ponta ao vivo: criei uma vaga com 2 benefícios, abri o drawer (badges verdes corretos), editei o título e adicionei um terceiro... digo, um segundo benefício via o novo diálogo de edição, confirmei no banco que `benefits` persistiu como `{Vale-refeição,"Plano de saúde"}`; simulei uma candidatura pública (arquivo PDF sintético) e confirmei que o input de arquivo e o estado local zeram sozinhos após o sucesso; e confirmei visualmente que um candidato sem vaga vinculada (`matchScore` nulo) agora mostra "0% de adesão" em vez de nenhum badge. Dados de teste (vaga, currículo, processo) removidos do banco local depois — sem mexer em dados de uso real que apareceram em paralelo no banco durante o teste (uma candidatura real recebida na vaga "Teste Link Vaga" enquanto eu testava).

## 10.36 Vincular candidato a uma vaga direto de "Analisar Candidatos"

- Nova ação "Vincular a uma vaga" (ícone de link) na coluna Ações da tabela de "Analisar Candidatos" — abre um modal ([`LinkCandidateToJobOpeningDialog.tsx`](src/features/resumes/components/../selection-processes/components/LinkCandidateToJobOpeningDialog.tsx)) pra escolher a vaga (reaproveitando o `JobOpeningPicker` já usado em "Vincular vaga" de um processo seletivo).
- **Backend**: novo endpoint `POST /api/v1/selection-processes/link-candidate` (`LinkCandidateToJobOpeningDto { resumeId, jobOpeningId }`) → `SelectionProcessService.linkCandidateToJobOpening()`, que reaproveita a mesma lógica já usada nas candidaturas públicas (`attachPublicApplication`): acha ou cria o processo seletivo `OPEN` daquela vaga (`"Candidaturas — {título}"`), adiciona o currículo como candidato via `upsert` (idempotente — se já for candidato do processo, não duplica nem sobrescreve a nota) e calcula o `matchScore` contra os requisitos/diferenciais da vaga. Log de auditoria (`SELECTION_PROCESS` · `ADD_CANDIDATE`). Recusa vagas que não pertencem à empresa do usuário ou que não estão mais `OPEN`.
- **Frontend**: hook `useLinkCandidateMutation`, rota BFF `POST /api/selection-processes/link-candidate`, e o botão/diálogo em [`CandidateTable.tsx`](src/features/resumes/components/CandidateTable.tsx) com toast de sucesso/erro.
- Aumentei a largura do container de "Analisar Candidatos" de `max-w-6xl` pra `max-w-7xl` ([`AnalyzeSection.tsx`](src/features/resumes/components/AnalyzeSection.tsx)) pra acomodar o botão extra sem apertar as colunas.
- **Validação**: `tsc --noEmit` + `nest build` (backend) e `eslint` + `tsc --noEmit` + `next build` (frontend) limpos. Testado ao vivo: vinculei um candidato já presente no processo (confirmei que o `upsert` não duplicou) e um candidato novo ("Anderson Silveira Araujo", sem match com os requisitos `react`/`next` da vaga) — o novo candidato entrou no processo com `matchScore: 0`, e o badge "0% de adesão" apareceu certinho ao abrir o processo (junto com a correção anterior de sempre mostrar o badge). Removi do banco local só o candidato de teste que eu mesmo adicionei, sem tocar nos dados reais já existentes no processo.
