...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { data } = await backendClient.patch(`/users/${id}/password`, body, withBearerToken(token));
    return NextResponse.json(data);
  } catch (error) {
    return forwardAxiosError(error, 'Não foi possível redefinir a senha do usuário.');
  }
}
