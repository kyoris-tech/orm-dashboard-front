...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { data } = await backendClient.delete(`/resumes/admin/${id}/permanent`, withBearerToken(token));
    return NextResponse.json(data);
  } catch (error) {
    return forwardAxiosError(error, 'Não foi possível excluir permanentemente o currículo.');
  }
}
